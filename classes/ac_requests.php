...
class AC_Requests
{
    ...
    public function get_banner($http_referer = null, $campaign_id = null)
    {
        ...
        if( !$http_referer && isset($_SERVER['HTTP_REFERER']) ) $http_referer = $_SERVER['HTTP_REFERER'];

        if( !$campaign_id && isset($_GET['campaign_id']) ) $campaign_id = $_GET['campaign_id'];

        if( !isset($http_referer) || empty($http_referer) ) return self::show_error(AC_API_ERROR_1);

        if( get_option('acs_active') !== '1' ) return self::show_error(AC_API_ERROR_9);
        if( !isset($campaign_id) || empty($campaign_id) || !is_numeric($campaign_id) ) return self::show_error(AC_API_ERROR_2);
        ...
        { // check if request comes from registered domain
            ...
            if( !$campaign_from_right_category ) return self::show_error(AC_API_ERROR_4);
        }

        if( (int) $campaign['status'] == 0 ) return self::show_error(AC_API_ERROR_5);

        // Making the return array

        unset($campaign['category_ids'], $campaign['category_title'], $campaign['categories']);

        if( $campaign['use_selected_banner'] && empty($campaign['selected_banner_id']) ) return self::show_error(AC_API_ERROR_6);

        if( AC_Data::ac_get_impressions_cnt($campaign['campaign_id']) >= $campaign['max_impressions'] && (int) $campaign['max_impressions'] > 0 ) return self::show_error(AC_API_ERROR_10);

        if( AC_Data::ac_get_clicks_cnt($campaign['campaign_id']) >= $campaign['max_clicks'] && (int) $campaign['max_clicks'] > 0 ) return self::show_error(AC_API_ERROR_11);
        ...
        if( !$campaign_active ) return self::show_error(AC_API_ERROR_13);
        ...
        if( !empty($campaign['selected_banner_link']) ) $ret_array['banner_link'] = $campaign['selected_banner_link'];
        elseif( !empty($campaign['link']) ) $ret_array['banner_link'] = $campaign['link'];
    ...
    {
        if( get_option('acs_active') === '1' ) echo json_encode(array('success' => '1', 'message' => 'CM Ad Changer Server is ON'));
        else echo json_encode(array('error' => '1', 'message' => 'CM Ad Changer Server is OFF'));
    }
