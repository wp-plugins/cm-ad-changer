...
class AC_Data
{
    ...
    function ac_handle_campaigns_post($data)
    {
        ...
        if( strlen($data['comment']) > 500 ) $errors[] = 'Note is too long';

        if( isset($data['campaign_id']) && !is_numeric($data['campaign_id']) ) $errors[] = 'Unknown campaign';

        if( $data['banner_display_method'] == 'selected' && !empty($data['banner_filename']) && empty($data['selected_banner']) ) $errors[] = 'Please select a banner';
        ...
        if( !empty($errors) ) return array('errors' => $errors, 'fields_data' => $data);

        // VALIDATIONS END
        // 1. Inserting the campaign record
        if( !isset($data['campaign_id']) )
        {
            $wpdb->query($wpdb->prepare('INSERT INTO ' . CAMPAIGNS_TABLE . ' SET `title`=%s, `link`=%s, `banner_display_method`=%s, `max_impressions`=%d, `max_clicks`=%d, `comment`=%s, `status`=%d', $data['title'], $data['link'], $data['banner_display_method'], $data['max_impressions'], $data['max_clicks'], $data['comment'], isset($data['status']) ? 1 : 0));

            $new_campaign_id = $wpdb->insert_id;
        }
        else
        {
            $wpdb->query($wpdb->prepare('UPDATE ' . CAMPAIGNS_TABLE . ' SET `title`=%s, `link`=%s, `banner_display_method`=%s, `max_impressions`=%d, `max_clicks`=%d, `comment`=%s, `status`=%d WHERE `campaign_id`="' . $data['campaign_id'] . '"', $data['title'], $data['link'], $data['banner_display_method'], $data['max_impressions'], $data['max_clicks'], $data['comment'], isset($data[status]) ? 1 : 0));
        ...
        if( isset($new_campaign_id) )
        {
            ...
            if( !empty($deleted_filenames) )
            {
                foreach($deleted_filenames as $deleted_filename)
                {
                    ...
                    {
                        if( file_exists(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . $deleted_filename) ) unlink(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . $deleted_filename);
                    }
        ...
        if( isset($data['banner_title']) && !empty($data['banner_title']) )
        {
            ...
            foreach($data['banner_title'] as $banner_index => $banner_title)
            {
                if( in_array($data['banner_filename'][$banner_index], $new_filenames) )
                {
                    ...
                    $wpdb->query($wpdb->prepare('INSERT INTO ' . IMAGES_TABLE . ' SET `campaign_id`=%d, `title`=%s, `title_tag`=%s, `alt_tag`=%s, `link`=%s, `weight`=%d, `filename`=%s', $new_campaign_id, $banner_title, $data['banner_title_tag'][$banner_index], $data['banner_alt_tag'][$banner_index], $data['banner_link'][$banner_index], $data['banner_weight'][$banner_index], $data['banner_filename'][$banner_index]));

                    if( $data['banner_filename'][$banner_index] == $data['selected_banner'] ) $selected_banner_id = $wpdb->insert_id;
                }else
                {
                    $wpdb->query($wpdb->prepare('UPDATE ' . IMAGES_TABLE . ' SET `title`=%s, `title_tag`=%s, `alt_tag`=%s, `link`=%s, `weight`=%d WHERE `filename`=%s', $banner_title, $data['banner_title_tag'][$banner_index], $data['banner_alt_tag'][$banner_index], $data['banner_link'][$banner_index], $data['banner_weight'][$banner_index], $data['banner_filename'][$banner_index]));
                    if( $data['banner_filename'][$banner_index] == $data['selected_banner'] ) $selected_banner_id = $wpdb->get_var('SELECT `image_id` FROM ' . IMAGES_TABLE . ' WHERE `filename`="' . $data['banner_filename'][$banner_index] . '"');
                }
            }
        }
        // cleaning tmp folder
        if( $handle = opendir(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . AC_TMP_UPLOAD_PATH) )
        {
            ...
            {
                if( file_exists(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . AC_TMP_UPLOAD_PATH . $entry) && $entry != '.' && $entry != '..' ) unlink(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . AC_TMP_UPLOAD_PATH . $entry);
            }
        }
    ...
    function ac_handle_settings_post($data)
    {
        ...
        if( empty($data['acs_max_campaigns_no']) ) $errors[] = 'Maximal Number of Campaigns is not set';

        if( !is_numeric($data['acs_max_campaigns_no']) ) $errors[] = 'Please specify valid Maximal Number of Campaigns';

        if( (int) $data['acs_max_campaigns_no'] > CAMPAIGNS_LIMIT ) $errors[] = 'Maximal Number of Campaigns must be less than ' . CAMPAIGNS_LIMIT;

        if( is_numeric($data['acs_max_campaigns_no']) && (int) $data['acs_max_campaigns_no'] < 1 ) $errors[] = 'Maximal Number of Campaigns must be greater than 1';

        $campaigns = self::ac_get_campaigns();
        if( count($campaigns) > $data['acs_max_campaigns_no'] ) $errors[] = 'Number of Campaigns in database is greater than specified Maximal Number of Campaigns';

        if( !empty($errors) ) return array('errors' => $errors, 'data' => $data);

        if( isset($_POST['acs_active']) ) update_option('acs_active', 1);
        else update_option('acs_active', 0);
    ...
    function ac_get_campaign($campaign_id)
    {
        ...
        if( $campaign->status == '1' ) $ret_fields['status'] = '1';
    ...
    function ac_remove_campaign($campaign_id)
    {
        ...
        foreach($images as $image) if( file_exists(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . $image) ) unlink(WP_CONTENT_DIR . '/uploads/' . AC_UPLOAD_PATH . $image);
    ...
    function get_history($page = 1, $output_type = OBJECT)
    {
        ...
        if( !is_numeric($page) || $page < 0 ) return array();
    ...
    function get_history_months()
    {
        ...
        $ret_months = array();
        foreach($months as $index => $month) if( $month->year == date('Y') ) $ret_months[] = $month->month;
            else $ret_months[] = $month->month . ', ' . $month->year;
        return $ret_months;
    }
    ...
    function get_history_month($month, $campaign_id = null)
    {
        ...
        if( count($month) == 1 ) $month[1] = date('Y');
    ...
    function get_clients_logs()
    {
        ...
        {
            foreach($client_domains as $domain) $clients_logs[] = $wpdb->get_row('SELECT h.referer_url, c.title as campaign_name, h.regdate, i.filename, i.title as banner_name FROM ' . HISTORY_TABLE . ' h
