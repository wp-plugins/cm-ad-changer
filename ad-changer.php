 ...
 */
define('ACF_PLUGIN_PATH', WP_PLUGIN_DIR . '/' . basename(dirname(__FILE__)));
define('ACF_PLUGIN_URL', plugins_url('', __FILE__));
define('ACF_PLUGIN_FILE', __FILE__);

require_once ACF_PLUGIN_PATH . '/config.php';
require_once ACF_PLUGIN_PATH . '/functions.php';
require_once ACF_PLUGIN_PATH . '/classes/ac_data.php';
require_once ACF_PLUGIN_PATH . '/classes/ac_requests.php';
require_once ACF_PLUGIN_PATH . '/classes/ac_client.php';

if( !class_exists('ACWidget') )
{
    require_once ACF_PLUGIN_PATH . '/widget.php';
}
...
register_deactivation_hook(__FILE__, 'acf_deactivate');
...
function ac_menu()
{
    ...
    $about_subpage = add_submenu_page('ac_server', 'About', 'About', 'manage_options', 'ac_server_about', 'ac_load_page');
    $pro_version_subpage = add_submenu_page('ac_server', 'Pro Version', 'Pro Version', 'manage_options', 'ac_server_pro_version', 'ac_load_page');
...
function ac_load_page($ac_page = null)
{
    ...
    if( empty($ac_page) ) $ac_page = $_GET['page'];

    $plugin_data = get_plugin_data(ACF_PLUGIN_FILE);


    $php_info = parse_php_info();
    if( (!isset($php_info['gd']) || !is_array($php_info['gd'])) && $ac_page != 'ac_server_about' )
    {
        $errors[] = 'GD library is not installed! Please install this module to use Ad Changer Server';
        require_once ACS_PLUGIN_PATH . '/views/error.php';
        return;
    }
    switch($ac_page)
    {
        ...
        case 'ac_server':
            if( !empty($_POST) )
            {
                ...
                if( !empty($data) )
                {
                    ...
                    if( isset($data['errors']) && !empty($data['errors']) ) $errors = $data['errors'];
                }else
                {
            ...
            require_once ACF_PLUGIN_PATH . '/views/settings.php';
            break;
        ...
        case 'ac_server_campaigns':
            ...
            require_once ACF_PLUGIN_PATH . '/views/campaigns.php';
            break;

        case 'ac_server_about':
            require_once ACF_PLUGIN_PATH . '/views/about.php';
            break;
        case 'ac_server_pro_version':
            require_once ACF_PLUGIN_PATH . '/views/pro_version.php';
            break;
    }
}
